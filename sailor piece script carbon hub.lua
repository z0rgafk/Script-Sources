...
getgenv().melissacarbon_Running = true
...
local theChosenOne = 139843228276286
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("fk melissacarbon • Session: %s • %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "fk melissacarbon • " .. os.date("%x %X") }
        }}
    }
...
local Window = Library:CreateWindow({
	Title = "fk melissacarbon",
	Footer = "" .. assetName .. " | melissacarbon | LOVE YOU",
	NotifySide = "Right",
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().melissacarbon_Running then break end
...
CreateSwitchGroup(-- auto title switch
TB_Tabs.Switch.T1, "Title", "Title", CombinedTitleList)
CreateSwitchGroup(-- auto rune switch
TB_Tabs.Switch.T2, "Rune", "Rune", Tables.RuneList)
CreateSwitchGroup(-- auto build switch
TB_Tabs.Switch.T3, "Build", "Build", Tables.BuildList)
...
SpecPassiveLabel = -- passive reroll
TB_Tabs.Enchant.T2:AddLabel("N/A", true)
...
ArtifactLabel = -- artifact manager
GB.Artifact.Left.Status:AddLabel("Status: N/A", true)
...
DungeonCount = -- dungeon config
TB_Tabs.Dungeon.T2:AddLabel("Dungeon Completed: N/A")
...
TB_Tabs.Dungeon.T2:AddToggle("DungeonAutofarm", {
    ...
    AddSliderToggle({ Group = -- player settings
GB.Player.Left.General, Id = "WS", Text = "WalkSpeed", Default = 16, Min = 16, Max = 250 })
...
MerchantTimerLabel = -- auto merchant
TB_Tabs.Merchant.T1:AddLabel("Refresh: N/A")
...
task.spawn(function()
    while getgenv().melissacarbon_Running do
...
MenuGroup:AddButton("Unload", function()
    getgenv().melissacarbon_Running = false
    Shared.Farm = false
    Cleanup(Connections)
    Cleanup(Flags)
	Library:Unload()
...
-- save config
ThemeManager:SetFolder("melissacarbon")
SaveManager:SetFolder("melissacarbon/SailorPiece")
